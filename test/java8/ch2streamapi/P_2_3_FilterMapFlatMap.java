...
public class P_2_3_FilterMapFlatMap {
    ...
    public void StreamsCanNotBeReused() throws Exception {
        ...
        final Stream<String> stream = Demo.newMutableList().stream();
    ...
    public void filter() throws Exception {
        ...
        Demo.newMutableList().stream().filter(f);
    }

    @Test
    public void map() throws Exception {
        //stream not of String, but of char
        final Stream<Character> stream = Demo.newMutableList().stream().map(s -> s.charAt(0));
    }

    @Test
    public void flatMap() throws Exception {
        final Stream<char[]> charArrayStream = Demo.newMutableList().stream().map(s -> s.toCharArray());
